...
def test_no_text_copies_through(tmp_path):
    clip = tmp_path / "in.mp4"
    out = tmp_path / "out.mp4"
    _make_clip(clip)
    add_text_to_video(str(clip), str(out))
    assert out.exists() and out.stat().st_size > 0


def test_no_leftover_overlay_png(tmp_path):
    clip = tmp_path / "in.mp4"
    out = tmp_path / "out.mp4"
    _make_clip(clip)
    add_text_to_video(str(clip), str(out), top="HI")
    assert not (Path(str(out) + ".overlay.png")).exists()
